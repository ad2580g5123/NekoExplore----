...
//０で１００?　通常１で２００?　－１で５０?
void KdSoundInstance::SetPitch(float val)
{
	if (m_instance == nullptr)return;


	m_instance->SetPitch(val);
}
...
std::shared_ptr<KdSoundInstance> KdSoundEffect::CreateInstance(bool b3D)
{
	...
	// SoundInstanceはprivateコ?スト?クタなので、make_sharedを使わない方法で作成する
...
void KdAudioManager::Update()
{
	...
	// 再生?スト??
	{
		...
		while(it != m_playList.end())
		{
			// 再生?でないときは、?ストから削?(どこからも参照されていなければ解放される)
			if(!(*it).second->IsPlay())
			{
